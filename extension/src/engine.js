/* The slop-chop wasm engine, hosted in whatever context can run it: a hidden offscreen
   document on Chrome, or the background page itself on Firefox, which has no offscreen API.
   It exposes self.slopEngine with boot, chop, and presets. The wasm boot mirrors the site
   worker, since the same binary backs both. wasm_exec.js must load before this. */
"use strict";
(function (global) {
  let ready = null;
  let defaults = null;
  ...
  function boot() {
    ...
    ready = (async () => {
      ...
      defaults = JSON.parse(global.slopDefaults());
    })().catch((err) => {
      ready = null;
      throw err;
    });
  ...
  // phraseReplace when it is several. Voice wins, and because the engine applies allow to
  // every rule, a kept word survives even a preset that would swap it.
  ...
  // chop runs the engine with the voice folded in and the presets on top. Saved preset names
  // are filtered against the engine's current packs, so a name removed by an update degrades
  // gracefully instead of erroring every chop until the user re-saves. boot must resolve first.
  function chop(text, settings) {
    const s = settings || {};
    const profile = voiceProfile(defaults, s.voice || {});
    const known = new Set(JSON.parse(global.slopPresets()));
    const presetList = (s.presets || ["cleaver"]).filter((p) => known.has(p));
    const req = JSON.stringify({ text, profile, presets: presetList });
    const res = JSON.parse(global.slopChop(req));
  ...
  // presets returns the engine's built-in preset names. boot must resolve first.
  function presets() {
    return JSON.parse(global.slopPresets());
  }

  global.slopEngine = { boot, chop, presets };
})(self);
